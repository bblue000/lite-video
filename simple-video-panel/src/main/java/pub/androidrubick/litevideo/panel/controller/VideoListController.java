...
public class VideoListController extends VideoController implements AbsListView.OnScrollListener {
    ...
    public interface VideoListCallback {
        /**
         * 如果不是含有视频的项，返回null；如果是含有视频的项，则返回{@link VideoView}
         */
        VideoView getVideoView(int position, View convertView);
    }
...
//        VideoView video = mVideoListCallback.getVideoView(position, child);
    }
    ...
    protected VideoView findCurrentPlayVideo() {
        ...
        for (int i = 0; i < mListView.getChildCount(); i++) {
            ...
            VideoView video = mVideoListCallback.getVideoView(firstItemPos + i, child);

            if (null != video) if (DEBUG) Log.w(TAG, "current play item：" + (firstItemPos + i + 1));
